...
export const PostService = {
    create: async(newPost, result) => {
        sql.query("INSERT INTO posts2 SET ?", newPost, (err, res) => {
    ...
    findById: async(postId, result) => {
        sql.query(
            `SELECT * FROM posts2 WHERE id = ?`, [postId],
            (err, res) => {
                if (err) result(err, null);
    ...
    getAll: async(result) => {
        sql.query("SELECT * FROM posts2", (err, res) => {
    ...
    updateById: async(id, post, result) => {
        sql.query(
            "UPDATE posts2 SET ? where id= ?", [post, id],
            (err, res) => {
                if (err) result(null, err);
    ...
    remove: async(id, result) => {
        sql.query("DELETE FROM posts2 WHERE id = ?", id, (err, res) => {
            if (err) result(null, err);
            else if (res.affectedRows == 0) result({ message: "post not found" }, null);
            else result(null, res);
        });
    ...
    removeAll: async(result) => {
        sql.query("DELETE FROM posts2", (err, res) => {
            if (err) result(null, err);
            else result(null, res);
        });
    },
